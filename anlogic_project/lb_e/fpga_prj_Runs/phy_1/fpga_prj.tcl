...
set rc [catch {
  open_project {fpga_prj.prj}
  import_device ph1_90.db -package PH1A90SBG484 -speed 2
  import_db {../syn_1/fpga_prj_gate.db}
  place
  export_db {fpga_prj_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file fpga_prj_phy.area
  write_verilog simulation/fpga_prj_phy_sim.v
  export_db {fpga_prj_pr.db}
  start_timer
  report_timing -mode FINAL -net_info  -rpt_autogen true -file fpga_prj_phy.timing -sdf simulation/fpga_prj.sdf
  write_verilog -sdf "fpga_prj.sdf" simulation/fpga_prj_phy_sim_sta.v
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid fpga_prj_inst.bid
  bitgen -bit "fpga_prj.bit" -compress -dispersed
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
