...
set rc [catch {
  ...
  elaborate -top {top}
  export_db {fpga_prj_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../fpga_pin.adc
  optimize_rtl
  write_verilog simulation/fpga_prj_rtl_sim.v
  report_area -file fpga_prj_rtl.area
  export_db {fpga_prj_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  optimize_gate -mapsim simulation/fpga_prj_map_sim.v -packsim simulation/fpga_prj_gate_sim.v -maparea fpga_prj_gate.area
  legalize_phy_inst
  export_db {fpga_prj_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
